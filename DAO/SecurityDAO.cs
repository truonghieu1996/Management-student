...
namespace DAO
{
    public class SecurityDAO
    {
        ...
        {
            string query = string.Format("BACKUP DATABASE QuanLyHocVien TO DISK='{0}'", strpath);
            try
            {
        ...
        {
            string query = string.Format("USE master RESTORE DATABASE QuanLyHocVien FROM DISK='{0}'", strpath);
            try
            {
                DataProvider.Instance.ExecuteNonQuery(query);
            }
